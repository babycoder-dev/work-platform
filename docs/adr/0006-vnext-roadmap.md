...
> **2026-07-07 增补拍板（产品负责人）：LLM 通道翻转——采购线上 API 为主通道，放弃内网
> 自部署 LLM**。连带变化：①"内网 LLM 推理基线评估"spike **取消**（其 go/no-go 判定被
> 本拍板前置替代，走的正是该 spike 预设的"云通道为主"退路）；② M15 的部署前置从"GPU/
> 模型/推理服务到位"变为"到 LLM API 提供方的受控公网 egress + API key 管理"；③ air-gapped
> 部署下 **agent 能力不可用**（平台其余功能不受影响）——对 constitution §1 前提修正的口径
> 相应加强；④ **"哪些数据类别可进 prompt 发给云 API"从可选通道的开启项升级为主通道的
> 硬边界**，归 Agent 身份子 ADR（本 ADR §4.3）新增拍板位。本节以下正文中与内网 LLM 相关
> 的表述以本注记为准，正式措辞随 Agent 子 ADR 收口。
...
2. LLM 双通道——云 API（Claude）与内网自部署（OpenAI 兼容端点）皆可切换
   （**2026-07-07 修正：云 API 为主通道，内网自部署放弃**，见状态节增补拍板）；
3. 开源接入姿态按组件混合决策；
...
| M20+ 持续项 | 🔧/📦 预留桶 | gateway 真拆分、桌面 Qt、多层部门、周报、Excel 导入、看板高级筛选、日报 git-diff、内网交付强化；按下方逐项触发条件插入，不阻塞主线 |

两条贯穿原则：① 无用户可见交付的纯 🔧 里程碑仅 M12、M17，其获得感风险已显式接受；
② M15 之后 `agentTools` 是模块 manifest 的常规组成部分（如 menus/permissions）。

M20+ 由本 ADR 维护完整清单与触发条件，其他文档只按自身职责投影：

- **gateway 真拆分**：用于独立扩缩容和服务自治；当组合宿主阻碍独立部署、容量隔离或团队自治时触发。
- **桌面 Qt**：用于原生性能、系统集成与低配体验；当 Web 无法满足已确认的原生性能或系统集成需求时触发。
- **多层部门**：用于表达真实组织层级；当企业组织出现必须落库和授权的二级以上部门时触发。
- **周报**：用于跨日汇总与管理复盘；当日报稳定交付且业务确认需要周粒度汇总时触发。
- **Excel 导入**：用于员工等批量数据录入；当人工录入无法满足已确认的批量导入规模时触发。
- **看板高级筛选**：用于大名册的组合查询；当现有筛选无法满足已确认的管理查询场景时触发。
- **日报 git-diff**：用于把代码变更纳入日报辅助信息；当代码仓接入范围与数据授权口径确定时触发。
- **内网交付强化**：用于镜像裁剪及安装、升级、回滚演练；进入正式离线生产交付验收前触发。
...
（M13）、Agent 运行时评估（M15）、~~内网 LLM 推理基线评估（M15 部署前置）~~（**2026-07-07
拍板取消**：改采线上 API，见状态节增补拍板）、Teable 解剖
（M17-M18）；OpenIM spike 的资源实测汇总为容量规划输入。
...
- **Agent = 数字员工**：常驻实例为平台一等实体（`agent.*` 状态机 registered→provisioning→
  ...
  **写操作确认平台锚定**（IM 卡片只载深链；IM 内联确认【预留：Agent 身份 ADR 论证把
  OpenIM 纳入确认 TCB 的条件后方可启用】）。
...
- **运维与算力（2026-07-06 缺漏审计后认领）**：M12 认领**最小可观测性基线**（告警带外
  ...
  部署前置~~（**2026-07-07 拍板改线上 API**：M15 部署前置变为受控 egress + API key 管理，
  LLM spike 取消）；deployment
  扩为"vNext 部署基线与容量规划"（统一备份矩阵 PG+files+Redis+Mongo/MinIO+agent PV、
  单机堆叠 vs 拆机容量视角、离线交付链路含 k3s air-gap——~~与模型权重导入~~，借 M13 立项
  欠账的 `docs/offline-deployment-runbook.md`）。
...
- **constitution §1**：
  ...
  - "系统必须支持企业内网无公网环境部署" → 平台本体保持内网可部署底线，但（2026-07-07
    拍板后）M15 起 **agent 能力依赖到云 LLM API 的受控公网 egress**；air-gapped 部署下
    agent 能力不可用。可发送数据类别的硬边界归 Agent 身份子 ADR 拍板；风险登记 spec §14。
...
搬运缝合成本不确定（spike 先行）；**M15 agent 能力对云 LLM API 的可用性/成本/数据边界
依赖**（2026-07-07 拍板取代原"内网硬件不到位"风险：API 中断则 agent 降级、按量成本须
预算、可发送数据类别硬边界归 Agent 子 ADR）；**owner 离职后数字员工失主**（Agent 身份 ADR 登记
