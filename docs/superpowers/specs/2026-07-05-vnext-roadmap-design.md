...
| LLM 连接性 | **双通道都要**（云 API + 内网自部署） | 由 pi-ai 满足，不自研 provider 抽象。**云通道构成对 constitution §1"内网无公网部署"前提的修正**：云通道为可选配置，air-gapped 部署降级为仅内网通道——ADR-0006 显式声明（§13），数据出内网风险登记 §14 |
...
1. **LLM 层 = pi-ai**（放弃自研 `@work/llm-provider`），外包一层部署配置（模型清单/密钥/
   ...
   **内网 LLM 端点是 M15 的部署前置（缺漏审计 P0）**：GPU 服务器规格、候选模型（中文
   能力/工具调用能力）、推理服务选型（vLLM 等 OpenAI 兼容形态）、离线权重导入——由专项
   spike"内网 LLM 推理基线评估"（§13.9，M15 前置）实证；硬件不到位则缺省通道不存在，
   风险登记 §14。
...
| **业务数据出内网**（云 LLM 通道发送在位/待办/审批等上下文至云 API） | 内网自部署通道为缺省；云通道显式开启 + 开启时声明可发送的数据类别；air-gapped 部署仅内网通道（ADR-0006 声明前提修正） |
