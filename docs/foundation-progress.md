...
| M7 通知 + 定时任务调度      | 自研站内通知 + 调度                                                                                     | In Progress | M7-3 调度基建已交付；下一步 M7-4 SSE 推送 + 前端铃铛 / 工作台卡片；不引入 IM/重型实时               |
...
①② 日报提醒预留接线点。下一步进入 M7-4 SSE 推送与前端铃铛 / 工作台卡片接入。
...
M7-2 事件订阅 + RecipientResolver + platform 读端口已完成；M7-3 调度基建已完成，下一步执行 M7-4；
forms UI 迁 M8。
...
| M7-4 | SSE 推送 + 前端铃铛 / 工作台卡片 | Pending | 下一步；REST 仍为事实源，触发点配置 UI 同步进入 M7-4 |
