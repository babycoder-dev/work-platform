...
| M9-3a | 看板实时化后端（§7.5 收口）   | In Progress | 任务包 `docs/tasks/m9-3a-board-realtime-backend.md` 已就绪（两轮独立审查全落修，待 Codex 实现）：getBoard 数据来源反转为名册 LEFT JOIN 活跃离岗记录（无记录=在岗缺省）+ 扩 `PlatformEmployeeLookupPort.listEmployeesByScope`（平台只读端口面）+ 实时部门过滤（弃记录快照）+ 看板响应新 `PresenceBoardEntryDto` 随行下发 statusLabel；安全敏感（读端口扩面 + 过滤依据变更，合并前强制 security-reviewer）；⚠️ 看板响应形态变更有 M9-3a→M9-3b web 回归窗口，须成对/紧接交付                                                                               |
