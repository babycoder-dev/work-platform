...
| M7 通知 + 定时任务调度      | 自研站内通知 + 调度                                                                                     | In Progress | M7-4a SSE 后端推送端点已交付；下一步 M7-4b 前端铃铛 / 工作台卡片；不引入 IM/重型实时                |
...
M7-2 事件订阅 + RecipientResolver + platform 读端口已完成；M7-3 调度基建已完成，M7-4a SSE 后端已完成，下一步执行 M7-4b；
forms UI 迁 M8。
...
| M7-4b | 前端铃铛 / 工作台卡片 + SSE 消费 | Pending | 下一步；断线回退 REST 轮询，触发点配置 UI 同步进入 M7-4b |
