...
forms 记录。**M9-3a 看板实时化后端任务包已就绪**（`docs/tasks/m9-3a-board-realtime-backend.md`，两轮独立审查
全落修；数据来源反转为名册 LEFT JOIN 活跃离岗记录、扩 `listEmployeesByScope` 平台只读端口、看板响应
`PresenceBoardEntryDto` 下发 statusLabel；安全敏感，合并前强制 security-reviewer；有 M9-3a→M9-3b web 回归窗口
须成对交付），待 Codex 实现。
...
| M9-3a | 看板实时化后端（§7.5 收口）   | Done    | 本分支实现，待 PR / security-reviewer 合并门：getBoard 数据来源反转为名册 LEFT JOIN 活跃离岗记录（无记录=在岗缺省）+ 扩 `PlatformEmployeeLookupPort.listEmployeesByScope`（平台只读端口面）+ 实时部门过滤（弃记录快照）+ 看板响应新 `PresenceBoardEntryDto` 随行下发 statusLabel；默认 e2e 已覆盖越权不泄露、department_tree、换部门实时归属、forms 值不下发；已知性能上限：company scope 走全量员工 + 全量活跃记录内存 join + 全员 id `user_id = ANY(...)` 下推 PG，大规模企业（>10k 员工）未测，优化留后续；⚠️ 看板响应形态变更有 M9-3a→M9-3b web 回归窗口，M9-3a 禁止单独合入生产/发布分支，须与 M9-3b 成对或紧接交付 |
