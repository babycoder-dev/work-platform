...
M9: 在位状态 v2（RFC docs/rfc/m9-presence-v2.md 已 Accepted 2026-07-01，两轮独立评审 + 三项拍板：
  ...
  └ M9-1 已实现并完成本机 PostgreSQL 全矩阵，进入 PR 交付审查（security-reviewer 由维护者执行）
  └ 下一步：M9-2（M9-1 security-reviewer + 合并完成后启动）
```
...
| M9-1  | 状态字典后端                  | In Review | 2026-07-03 实现与本机门禁完成；`presence.status_types`（is_default partial unique index）+ 七端点 archive-only 管理（无硬删）+ 记录 status 放宽（DROP status CHECK、保留 time-range CHECK）+ 三类拒登 + 动态 default 豁免 + `form_record_id` + statusLabel/M7 消费；`test:db` 5/37、PG e2e 3/15 真跑，待维护者 security-reviewer 后置 Done |
