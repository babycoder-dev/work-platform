...
export async function runNotificationMigrations(): Promise<void> {
  ...
  try {
    ...
    for (const migration of migrationFiles) {
      ...
      try {
        ...
        await client.query('INSERT INTO notification.schema_migrations (name) VALUES ($1)', [
          migration.name,
        ]);
        await client.query('COMMIT');
        console.log(`Applied notification migration ${migration.name}`);
      } catch (error) {
...
async function ensureMigrationTable(client: Client): Promise<void> {
  await client.query('CREATE SCHEMA IF NOT EXISTS notification');
  await client.query(`
...
async function listAppliedMigrations(client: Client): Promise<Set<string>> {
  const result = await client.query<{ name: string }>(
    'SELECT name FROM notification.schema_migrations',
  );
...
if (process.argv[1] && path.resolve(process.argv[1]) === modulePath) {
  runNotificationMigrations().catch((error: unknown) => {
    console.error(error);
    process.exitCode = 1;
  });
}
