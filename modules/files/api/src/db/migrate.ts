...
export async function runFilesMigrations(): Promise<void> {
  ...
  try {
    ...
    for (const migration of migrationFiles) {
      ...
      try {
        ...
        await client.query('INSERT INTO files.schema_migrations (name) VALUES ($1)', [migration.name]);
        await client.query('COMMIT');
        console.log(`Applied files migration ${migration.name}`);
      } catch (error) {
...
async function ensureMigrationTable(client: Client): Promise<void> {
  await client.query('CREATE SCHEMA IF NOT EXISTS files');
  await client.query(`
    CREATE TABLE IF NOT EXISTS files.schema_migrations (
...
async function listAppliedMigrations(client: Client): Promise<Set<string>> {
  const result = await client.query<{ name: string }>('SELECT name FROM files.schema_migrations');
...
if (process.argv[1] && path.resolve(process.argv[1]) === modulePath) {
  runFilesMigrations().catch((error: unknown) => {
    console.error(error);
    process.exitCode = 1;
  });
}
